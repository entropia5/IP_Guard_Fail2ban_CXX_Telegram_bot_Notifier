...
Уточнение: Ещё не упомянул выше библиотеку nlohmann/json.
Если она не была установлена у вас, необходимо установить.

sudo apt install nlohmann-json3-dev

Она используется для парсинга данных (IP)

В дальнейшем на досуге добавлю URL Encoding для текста сообщения, чтобы переводить спецсимволы в формат, который понимает интернет браузер или сервер. Возможно это и не понадобиться, потому что libcurl имеет ф-цию для этого. Можно прогнать текст через curl_easy_escape. К примеру так:
/*
char* output = curl_easy_escape(curl, message.c_str(), message.length());
if(output) {
    std::string encodedMessage = output;
    curl_free(output);
}
*/

И есть ещё пару интересных идей, внедрить асинхронность и фичи с доменами, логи в файл, карту в сообщениях бота телеграм, которая даст координаты по широте и долготе, плюс хочу добавить базу данных, чтоб хранить историю банов локально. Однако это займёт время, а в сутках как всегда его мало.
